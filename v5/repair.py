...
from v5.models import Ok, Rejected, Result, R_AGGREGATE_INVALID, R_FABRICATION, R_NOT_FOUND, R_STALE_REPAIR
from v5.obligations import transfer_subtree_obligations_to_uor_locked
from v5.store import Store, jdump, utcnow
...
_REPAIRABLE_FIELDS = {
    ...
    "plan": {"status", "integrity", "superseded_by", "dependency_graph"},
    "step": {"status", "integrity", "required", "depends_on"},
    "action": {"status", "integrity", "confirmation_id"},
    "verification": {"result", "integrity"},
    "obligation": {"owner", "disposition", "integrity"},
}
...
def abandon_unrepairable(store: Store, auth: HumanAuthorization, obj_id: str,
    ...
    with store.write() as conn:
        ...
        if kind is None:
            return Rejected(R_NOT_FOUND, f"object {obj_id}", None)
        row = conn.execute(f"SELECT * FROM {_TABLES[kind]} WHERE id = ?", (obj_id,)).fetchone()
        if row["integrity"] == "ABANDONED_UNREPAIRABLE":
        ...
        store.audit(
            conn, obj_id, "abandoned_unrepairable", row["integrity"], "ABANDONED_UNREPAIRABLE",
...
def _validate_aggregate(conn, kind: str, row, target_state: dict) -> Rejected | None:
    ...
    if kind == "verification" and "result" in target_state:
        ...
        if new_result == "PASS" and row["result"] not in ("PENDING", "RUNNING", "PASS"):
            # setting PASS from a non-evaluated state presumes an evaluation
            # that did not happen — same fabrication class as Law 29.
            return Rejected(R_FABRICATION,
                            "verification PASS repair requires an evaluable prior state", dict(row))
    return None
